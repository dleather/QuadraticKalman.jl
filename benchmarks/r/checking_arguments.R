...
check.transition <- function(Mu.t, Phi.t, Omega.t, observable) {
      ...
      if(dim(Mu.t)[3]!=time.length |
                      dim(Phi.t)[3]!=time.length |
                      dim(Omega.t)[3]!=time.length) {
            stop("if transition parameters are time-varying, third dimension must be equal to the time length.")
            
      }
      
      
      # Check missing, null or inf values
...
check.measurement <- function(A.t, B.t, C.t, D.t, observable, n) {
      ...
      if(!inherits(D.t, "matrix") && !inherits(B.t, "array")){
      ...
      # Check time-varying
      if(is.na(dim(A.t)[3])==TRUE | is.null(dim(A.t)[3])==TRUE | dim(A.t)[3]==1){
            A.t <- array(1,c(1,1,time.length)) %x% matrix(A.t, nrow = m, ncol = 1)
      } 
      if(is.na(dim(B.t)[3])==TRUE | is.null(dim(B.t)[3])==TRUE | dim(B.t)[3]==1){
            B.t <- array(1,c(1,1,time.length)) %x% B.t
      } 
      if(is.na(dim(C.t)[4])==TRUE | is.null(dim(C.t)[4])==TRUE | dim(C.t)[4]==1){
            C.t <- array(1,c(1,1,1,time.length)) %x% C.t
      }
      if (is.na(dim(D.t)[3])==TRUE | is.null(dim(D.t)[3])==TRUE | dim(D.t)[3]==1 ) {
