...
QKF <- function(Z0, V0, Mu.t, Phi.t, Omega.t, A.t, B.t, C.t, D.t, observable) {
      ...
      for (t in 1:time.length) {
            ...
            # Check if eig$vectors has a dim attribute; if not, coerce it into a 1x1 matrix
            #print("eig$vectors")
            #print(eig$vectors)
            #print("eig$values")
            #print(eig$values)
            #print("diag(c(eig$values))")
            #print(diag(c(eig$values)))
            #print("solve(eig$vectors)")
            #print(solve(eig$vectors))
            if (length(eig$values) == 1) {
                implied.corrected.vcov <- eig$vectors * eig$values[eig$values >= 0] * solve(eig$vectors)
            } else {
