...
A data structure for holding an `M x T_bar` array of real values (`Y`)
plus two integer fields (`M`, `T_bar`) derived from its dimensions. `M` is 
the dimension of the measurement vector, 'Y_t', and `T_bar` is the 
number of time periods minus 1 (due to the autoregressive structure of the model).
