...
- **`QKData{T<:Real,N}`**  
  ...
  - `T̄::Int`: Number of time steps minus 1.

- **Constructor**:  
  `QKData(Y::AbstractArray{T,N})` automatically infers `M` and `T̄`
...
- `T̄::Int` : One less than the “second dimension” in a 2D case, or `length(Y) - 1` for a 1D vector.
"""
@with_kw struct QKData{T<:Real, N}
    Y::AbstractArray{T, N}  # Data array
    M::Int                  # Number of observed variables
    T̄::Int                  # Number of time periods minus 1
end
...
For matrix input (N=2):
  ...
  - T̄ is second dimension size minus 1
"""
function QKData(Y::AbstractArray{T, N}) where {T<:Real, N}
    ...
    data = QKData{T, N}(Y = Y, M = M, T̄ = Tp1 - 1)
...
- T̄ matches data length minus 1
"""
function validate_data(data::QKData{T,N}) where {T<:Real, N}
    ...
    if N == 1
        ...
        if T̄ != length(Y) - 1
            throw(ArgumentError("T̄ must equal length(Y) - 1"))
        end
    else
        if size(Y,2) ≤ 1
        ...
        if T̄ != size(Y,2) - 1
            throw(ArgumentError("T̄ must equal number of time periods minus 1"))
        end
    end
...
function get_measurement(data::QKData{T,N}, t::Int) where {T<:Real, N}
    ...
    if t < 1 || t > T̄ + 1
        throw(ArgumentError("Time index t must be between 1 and T̄+1"))
    end
