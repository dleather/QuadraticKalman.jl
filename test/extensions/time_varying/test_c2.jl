...
function simulate_integrals(eprob, N_run, N_boot, θ_y, Δt)
    estimates = zeros(N_boot, N_run)
    estimates_sq = zeros(N_boot, N_run)
    @floop for j = 1:N_run
        esol = solve(eprob, EM(); dt = Δt/400, trajectories = N_boot)
...
I1_sim, I2_sim =  simulate_integrals(eprob, N_run, N_boot, θ_y_init, Δt)
