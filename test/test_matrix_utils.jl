using Revise
using LinearAlgebra, Random, Test, QuadraticKalman

@testset "Matrix Utils Tests" begin
    @testset "selection_matrix" begin
        S = selection_matrix(3)
        @test size(S) == (6, 9)
    ...
    @testset "make_positive_definite" begin
        A = [1.0 0.2; 0.2 1.0]
        # already PD
        A_pd = QuadraticKalman.make_positive_definite(A)
        @test isposdef(A_pd)
        ...
        B = [ 1.0 0.99
              0.99 0.98 ]
        @test !isposdef(B)
        B_pd = make_positive_definite(B)
        @test isposdef(B_pd)
    end

    @testset "spectral_radius" begin
        M = [0.5 0.0; 0.0 0.9]
        @test spectral_radius(M) ≈ 0.9
    end
end
